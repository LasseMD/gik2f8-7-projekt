...
function saveTask() {
  ...
  api.create(task).then((task) => {
    ...
    översätter igen till ett JavaScript-objekt, och till sist returnerar som ett promise. Nu har äntligen det promiset fångats upp och dess 
    innehåll - uppgiften från backend - finns tillgängligt och har fått namnet "task".  */
    if (task) {
      /* När en kontroll har gjorts om task ens finns - dvs. att det som kom tillbaka från servern faktiskt var ett objekt kan vi anropa renderList(),
       som ansvarar för att uppdatera vår todo-lista. renderList kommer alltså att köras först när vi vet att det gått bra att spara ner 
       den nya uppgiften.  */
      renderList();
    }
  });
...
function renderBookList(bookList) {
  ...
  for (let i = 0; i < element.length; i++) {
    element[i].addEventListener("click", async (e) => {
      const book = await getBookDetails(e.target.id);
      saveBook(book);
    });
  }
}
